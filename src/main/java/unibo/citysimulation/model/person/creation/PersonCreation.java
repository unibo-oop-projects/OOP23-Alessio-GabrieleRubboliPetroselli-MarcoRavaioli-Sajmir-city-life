...
public final class PersonCreation {
    ...
    private static List<DynamicPerson> createGroupOfPeople(final int groupCounter, final int numberOfPeople,
        ...
        for (int i = 0; i < numberOfPeople; i++) {          
            Optional<Business> assignedBusiness = Optional.empty();
            String personName = "Person" + groupCounter + i;
            int personAge = random.nextInt(ConstantAndResourceLoader.MAX_RANDOM_AGE) + ConstantAndResourceLoader.MIN_AGE;
            int personMoney = random.nextInt(moneyMinMax.getSecond() - moneyMinMax.getFirst()) + moneyMinMax.getFirst();
            DynamicPerson person = createPerson(personName, personAge, assignedBusiness, residenceZone, personMoney);
            people.add(person);
        }

        for (DynamicPerson person : people) {
            for (Business b : businesses) {
                if(b.hire(new Employee(person, b.getBusinessData())) && !b.getBusinessData().zone().equals(person.getPersonData().residenceZone())){
                    person.getPersonData().business().of(b);
                }
            }
        }
        return people;       
    ...
    private static DynamicPerson createPerson(final String name, final int age, final Optional<Business> business,
            final Zone residenceZone, final double money) {
